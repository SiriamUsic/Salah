...
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        try:
            ...
            if b.status == "kicked":
                await m.reply_text(
                    f"@{ASSISTANT_NAME} **محظور في المجموعه** {m.chat.title}\n\n» **قم بفك خظر المستخدم اولا او تأكد من تقييد الحساب المساعد.**"
                )
                return
        ...
        if replied:
            if replied.audio or replied.voice:
                suhu = await replied.reply("**انتظر جاري البحث.**")
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await suhu.delete()
                        await m.reply_text(f"🚫 تأكد من وجود مكالمه مفتوحه اولَآ:\n\n» {e}")
            else:
                if len(m.command) < 2:
                    await m.reply(
                        "رد على ملف صوتي او اكتب شيئا للبحثّ**"
                    )
                else:
                    ...
                    if search == 0:
                        await suhu.edit("❌ **لم يتم العثور على نتائجِ.**")
                    else:
                        songname = search[0]
                        url = search[1]
                        duration = search[2]
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                try:
                                    await suhu.edit("🔄 ** جاري انضمام المساعد للمكالمهَ...**")
                                    await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 تأكد من وجود مكالمه مفتوحه اولَآ: `{ep}`")
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                ...
                                await suhu.delete()
                                await m.reply_text(f"حدث خطأ غير متوقع ،، تأكد من وجود مكالمه مفتوحه اولا`{ep}`")
